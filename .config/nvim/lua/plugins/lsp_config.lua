return {
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(ev)
					...
					vim.keymap.set("n", "<leader>lr", function()
						vim.lsp.buf_detach_client(0, vim.lsp.get_clients()[1].id) -- Detach
	...
	},
	{
		...
		keys = {
			{
				-- Customize format keymap as needed
				"<leader>cf",
				function()
		...
		opts = {
			-- Define formatters & their configuration
			formatters_by_ft = {
				lua = { "stylua" },
				python = { "isort", "black" },
				php = { "php_cs_fixer" },
			...
			formatters = {
				php_cs_fixer = {
					-- Command to execute PHP CS Fixer inside your docker-compose container
					command = "docker",

					-- Arguments to pass to the command
					args = function(ctx)
						local real_path = ctx.real_file or ""
						local container_path =
							real_path:gsub(os.getenv("HOME") .. "/ApprenTIM_v_0/app", "/var/www/html")
						return {
							"compose",
							"exec",
							"-T",
					...
					-- Stdin is used for the file content
					stdin = true,

					-- No need for specific working directory requirements
					require_cwd = false,

					-- Return stdout instead of stderr for the formatted content
					to_stdin = true,

					-- Explicitly state we want the formatted content from stdout
					to_stdout = true,
				},
				beautysh = {
				...
				vim.api.nvim_create_autocmd("BufWritePre", {
					...
					callback = function(args)
						conform.format({
							...
							lsp_fallback = opts.format_on_save.lsp_fallback,
						})
					end,
				})
			end
	...
	-- Snippets
	{
		...
		config = function()
			...
			ls.config.set_config({
				history = true, -- Keep around last snippet to jump back
				updateevents = "TextChanged,TextChangedI", -- Update on every change
			...
			-- Optional: Reload snippets without restarting Neovim
